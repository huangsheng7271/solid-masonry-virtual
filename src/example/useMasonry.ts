...
const useMasonry = () => {
  ...
  const checkScrollPosition = async () => {
    ...
    const masonryWrap = document.querySelector(".masonry-wrap")
    if(!masonryWrap) return;
    const scrollHeight = masonryWrap.scrollHeight
    const scrollTop = masonryWrap.scrollTop
    const clientHeight = masonryWrap.clientHeight
  ...
  onMount(() => {
    ...
    const masonryWrap = document.querySelector(".masonry-wrap") as HTMLElement
    if(!masonryWrap) return;
    masonryWrap.style.height = window.innerHeight + 'px';
    
    const scrollHandler = useDebounceFn(() => {
      checkScrollPosition();
    },50)

    masonryWrap.addEventListener('scroll', scrollHandler);
    onCleanup(() => {
      masonryWrap.removeEventListener('scroll', scrollHandler);
    });
  })
